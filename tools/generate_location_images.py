...
Reads room ids + descriptions from:
    Murder/game/images/locations/locations.md   (a Markdown table)
...
Usage (from the repo root):
    python tools/generate_location_images.py                  # all rooms, skip ones already generated
    python tools/generate_location_images.py --force          # regenerate everything
    python tools/generate_location_images.py library kitchen  # only these ids
...
MD_FILE = ROOT / "Murder/game/images/locations/locations.md"
...
def generate_one(rid, desc, out_dir, args):
...
def main():
    ...
    ap.add_argument("--force", action="store_true", help="Regenerate even if <id>.png already exists.")
    ...
    for i, (rid, desc) in enumerate(rooms, 1):
        ...
        if out_path.exists() and not args.force:
            print(f"[{i}/{total}] skip   {rid} (exists)")
            skipped += 1
            continue
        print(f"[{i}/{total}] gen    {rid} ...", end="", flush=True)
        t0 = time.time()
        try:
            generate_one(rid, desc, out_dir, args)
            ...
            failed += 1
            break  # connection errors will repeat for every room
    ...
    print(f"\nDone — {generated} generated, {skipped} skipped, {failed} failed. Output: {out_dir}")
