label doctor_config_menu:
    ...
    $ broken_generic_menu_doctor = TimedMenu("broken_generic_menu_doctor", [
        ...
        # The questions below cost 0 on purpose: each one opens an offense
        # sub-menu (the reply choices) and the time is charged there
        TimedMenuChoice('Tell me more about yourself.', 'broken_generic_background', 0, linked_choice ="broken_generic_heroic_act", next_menu="broken_generic_background_offense", condition = not_broken_offended),
        TimedMenuChoice('Why were you invited here?', 'broken_generic_heroic_act', 0, next_menu="broken_generic_heroic_act_offended", condition = not_broken_offended + " and is_linked_choice_hidden('broken_generic_menu_doctor', 'broken_generic_heroic_act')"),
        TimedMenuChoice('What do you think of this place?', 'broken_generic_manor', 0, next_menu="broken_generic_manor_offense", condition = not_broken_offended),
        TimedMenuChoice('How old are you?', 'broken_generic_age', 10, condition = not_broken_offended),
        TimedMenuChoice('What room are you in?', 'broken_generic_room', 0, next_menu="broken_generic_room_offense", condition = not_broken_offended),
        TimedMenuChoice('What do you think of the other guests?', 'broken_generic_other_guests_friday', 0, next_menu="broken_generic_other_guests_friday_offense", condition = condition_friday + " and " + not_broken_offended),
