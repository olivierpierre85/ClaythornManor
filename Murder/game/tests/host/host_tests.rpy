testsuite host:

    testcase friday_afternoon:
        python:
            test.run_chapter(host_details, "friday_afternoon", "host_introduction")

    testcase friday_evening:
        python:
            test.run_chapter(host_details, "friday_evening", "host_day1_evening")

    testcase saturday_morning:
        python:
            test.run_chapter(host_details, "saturday_morning", "host_day2_morning")

    testcase saturday_afternoon:
        python:
            test.run_chapter(host_details, "saturday_afternoon", "host_day2_hunt")

    testcase saturday_evening:
        python:
            test.run_chapter(host_details, "saturday_evening", "host_day2_evening")

    testcase sunday_morning:
        python:
            test.run_chapter(host_details, "sunday_morning", "host_day3_morning")

    testcase sunday_afternoon:
        python:
            test.run_chapter(host_details, "sunday_afternoon", "host_day3_afternoon")
